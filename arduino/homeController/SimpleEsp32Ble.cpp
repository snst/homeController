...
void setBleAddr(esp_bd_addr_t addr, char* stringAddress) {
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        Serial.println("#ESP_GATTC_CONNECT_EVT");
...
Serial.println("set service search");
...
esp_ble_gattc_register_for_notify (gattc_if, gl_profile_tab[PROFILE_A_APP_ID].remote_bda, 0x421);
...
//            ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, receive notify value:");
          Serial.print("receive notify value: ");
          setMqttResponse(p_data->notify.value, p_data->notify.value_len);
        }else{
          Serial.print("receive indicate value: ");
...
//        ESP_LOGI(GATTC_TAG, "ESP_GATTC_SRVC_CHG_EVT, bd_addr:");
  //      esp_log_buffer_hex(GATTC_TAG, bda, sizeof(esp_bd_addr_t));
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
          Serial.println("#ESP_GATTC_WRITE_CHAR_EVT");
        if (p_data->write.status != ESP_GATT_OK){
       //   Serial.println("#");
...
void updateBLE(uint8_t bleCmd[], uint8_t* bleCmdLen) 
{
  ...
  if(*bleCmdLen>0)
  {
    ...
    if(doConnect) {
      ...
      setBleAddr(BLE_ADDR, "00:1a:22:0c:5e:15");
  
      esp_err_t errRc = esp_ble_gattc_open(gl_profile_tab[PROFILE_A_APP_ID].gattc_if, BLE_ADDR, true);
    
      if (errRc != ESP_OK) {
        Serial.print("Failed: esp_ble_gattc_open");
        return;
      }
      else {
        Serial.println("OK: esp_ble_gattc_open");
      }
    ...
    Serial.println("connected and cmd");
